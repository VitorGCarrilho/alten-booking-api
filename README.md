...
To deal with concurrence we are using the room number as the partition key. Kafka guarantees delivery order in the 
same partition! The first to book will be the firs to be consumed.

On the save in database, the book will be saved in the `booking` table and for each day it will be stored in the 
`booking_day`. This table haves the room number and the day of booking as primary key. It means that only one guest will
have the book accepted, the others will receive a feedback email saying that the period is already booked by someone. 

## Database

The database choice was made thinking about managed transactions. If 2 persons try to book the same data, for example: 
person 1 tries from 2021-12-16 until 2021-12-18 and person 2 tries from 2021-12-18 until 2021-12-20 only the first book 
will succeed, the other will be rolled back.

The tables are structured as the diagram:

![Alt text](readme-resources/database-diagram.png?raw=true "Title")

if the first one succeeds, there will be a row in `booking` with the success booked status, and will be 3 rows in 
`booking_day` one for each day of the reservation (2021-12-16, 2021-12-17 and 2021-12-18). For the second user that 
the book failed, there will be one record in `booking` with the failed booking status and no record on the 
`booking_day`. 

For production this sql database could be fully maintained by the hostel's IT department, or it could be a service 
on cloud such as <a href="https://aws.amazon.com/rds/?nc2=type_a" target="_blank">Amazon RDS</a> which will make 
easier to the IT department manage. The ideal scenario here is to have one master cluster (the only one that writes 
on database) and 2 slaves that can be used for read only propose. The data should be replicated through the slaves and 
if the master goes down, on slave is elected to the master. This will make our database more resilient and fault 
tolerant. 


## Apache Kafka

Kafka is distributed, fault-tolerant, scalable event streaming platform created in linkedin and adopted 
all <a href="https://kafka.apache.org/powered-by">over the world</a>

For production this kafka cluster could be fully maintained by the hostel's IT department, or it could be a service
on cloud such as <a href="https://aws.amazon.com/msk/" target="_blank">Amazon MSK</a> which will make
easier to the IT department manage.

With AWS you can your broker can be located in multiple availability zones to create a highly available cluster. 
That means even if godzilla try to destroy the server building, your application will still available in others zones.

![Alt text](readme-resources/godzilla-breaking-servers.gif?raw=true "Title")

## Redis

Redis is a Fast in-memory data store key-value.

Here we are using as a fallback if the database is not available.

For production this redis cluster could be fully maintained by the hostel's IT department, or it could be a service
on cloud such as <a href="https://aws.amazon.com/redis/" target="_blank">Amazon ElasticCache</a> which will make
easier to the IT department manage.

## Production

For production, we are expecting to deploy the application in multiples availability zones. It can be deployed in cloud 
services like 
<a href="https://aws.amazon.com/ec2/?ec2-whats-new.sort-by=item.additionalFields.postDateTime&ec2-whats-new.sort-order=desc">
Amazon EC2</a> or in containers managed by <a href="https://aws.amazon.com/eks/">Amazon EKS</a>. The advantage of using 
